...
class _BrowserWebViewState extends State<BrowserWebView> {
  ...
  /// 初始化代理服务
  Future<void> _initializeProxyService() async {
    try {
      ...
      // 设置初始代理状态
      _proxyEnabled = WebViewProxyService.instance.isProxyEnabled;
      
      debugPrint('WebView代理服务初始化完成');
    } catch (e) {
      debugPrint('WebView代理服务初始化失败: $e');
    }
  }
  ...
  void _onProxyConfigChanged(ProxyConfig? oldConfig, ProxyConfig? newConfig) {
    if (mounted) {
      setState(() {
        _proxyEnabled = newConfig?.enabled ?? false;
      });
      
      // 重新加载WebView以应用新配置
      _reloadWithProxyConfig();
    }
  }
  
  /// 使用代理配置重新加载WebView
  Future<void> _reloadWithProxyConfig() async {
    try {
      if (_webViewController != null) {
        ...
        if (currentUrl != null) {
          await _webViewController.loadUrl(
            urlRequest: URLRequest(url: WebUri.uri(currentUrl)),
          );
        }
      }
    } catch (e) {
      debugPrint('重新加载WebView失败: $e');
    }
  }
  ...
  /// 获取代理错误信息
  String? get proxyErrorMessage => _proxyErrorMessage;
  
  /// 测试当前代理连接
  Future<ProxyTestResult> testProxyConnection() async {
    return await WebViewProxyService.instance.testProxyConnection();
  }
  
  /// 切换代理启用状态
  Future<bool> toggleProxy() async {
    return await WebViewProxyService.instance.toggleProxy();
  }
}